...
def obtain_new_grasps(grasp_matrix):
    """
    Takes in a single grasp 4x4 matrix from contact graspnet, obtains target translation grasp point, 
    vary the pitch -+45 degrees along the the approach vector (the z axis of the gripper frame), 
    derotate the gripper with the new rotation matrix, to obtain the new translation to update the grasp matrix. 

    Args:
        grasp_matrix (np.ndarray): The grasp matrix from contact graspnet (4x4).

    Returns:
        new_grasp_matrices (np.ndarray): The new grasp matrix with updated translation and rotation.
    """
    #For our gripper, the z axis displacement from the contact graspnet calculated grasp point to the gripper frame is 75mm
    displacement = 0.09
    R_original = grasp_matrix[:3, :3]
    t_original = grasp_matrix[:3, 3]
    approach_vector_unit = R_original[:, 2]  # Z-axis of the gripper frame
    initial_z_axis = np.array([0, 0, 1])
    target_translation = t_original + displacement * approach_vector_unit
    initial_rotation = Rotation.from_matrix(R_original)
    current_euler = initial_rotation.as_euler('xyz', degrees=True)
    pitch = current_euler[1]
    new_pitch, new_grasp_matrices = [], [grasp_matrix]
    new_pitch.append(pitch + 45)
    new_pitch.append(pitch - 45)
    rotation_axis = np.cross(initial_z_axis, approach_vector_unit)

    for final_pitch_deg in new_pitch:
        if np.linalg.norm(rotation_axis) < 1e-6:
            final_rotation = Rotation.from_euler('xyz', [current_euler[0], final_pitch_deg, current_euler[2]], degrees=True).as_matrix()
        else:
            rotation_axis_unit = rotation_axis / np.linalg.norm(rotation_axis)
            delta_pitch_deg = final_pitch_deg - pitch
            pitch_change_rotation = Rotation.from_rotvec(delta_pitch_deg * rotation_axis_unit)
            final_rotation = pitch_change_rotation * initial_rotation
        new_approach_vector_unit = final_rotation.as_matrix()[:,2]
        new_translation = target_translation - displacement * new_approach_vector_unit
        new_grasp_matrix = np.eye(4)
        new_grasp_matrix[:3, :3] = final_rotation.as_matrix()
        new_grasp_matrix[:3, 3] = new_translation
        new_grasp_matrices.append(new_grasp_matrix)
    return new_grasp_matrices
