...
saved_dir = glasses.start_recording('./recordings')
...
data_81 = DataRecorder(frame_name=os.path.join(saved_dir, 'rgbcam_81'), coord_name=os.path.join(saved_dir, 'gaze_81'), frame_type="realsense")
data_56 = DataRecorder(frame_name=os.path.join(saved_dir, 'rgbcam_56'), coord_name=os.path.join(saved_dir, 'gaze_56'), frame_type="realsense")
...
while not quit_keypress():
    ...
    if rgb_image is not None:
        ...
        if realsense_image_81 is not None and np.array([x_81, y_81]).all() is not None:
            visualize_streaming(realsense_window_81, realsense_image_81, gaze_vis_params, [x_81, y_81], with_text=False)
            data_81.record_frame(realsense_image_81, (x_81, y_81)) 
        if realsense_image_56 is not None and np.array([x_56, y_56]).all() is not None:
            visualize_streaming(realsense_window_56, realsense_image_56, gaze_vis_params, [x_56, y_56], with_text=False)
            data_56.record_frame(realsense_image_56, (x_56, y_56)) 

        
data_56.end_recording()
data_81.end_recording()
