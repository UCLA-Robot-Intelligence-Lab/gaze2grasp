...
def segment_rgb_image(
            ...
            if len(mask_indices) > 0:
                ...
                if within_radius:
                    ...
                    if mask_size <= max_mask_ratio * image_size:  # Check if mask is not larger than 20%
                        ...
                        print("Appended mask")
    # Combine masks
    # Assuming input_img is in BGR format (OpenCV's default)
    combined_mask = np.zeros((input_img.shape[0], input_img.shape[1]), dtype=np.uint8)
    ...
    if filtered_masks:
        ...
        # Create 3-channel mask (works for both BGR and RGB)
        combined_mask_3c = cv2.merge([combined_mask, combined_mask, combined_mask])

        # Apply mask to original RGB image directly
        segmented_object_rgb = cv2.bitwise_and(input_img, combined_mask_3c)

        return segmented_object_rgb, filtered_centers

    else:
        return np.zeros_like(input_img), filtered_centers
